...
const fresh = () => ({phase:'LOBBY', questionIndex:-1, gameStartTime:null, questionStartsAt:null, questionEndsAt:null, auto:false, paused:false, players:{}, answers:{}, history:[]});
let game = fresh();
try { const saved=JSON.parse(fs.readFileSync(stateFile,'utf8')); game={...fresh(),...saved}; delete game.pin; } catch {}
const save = () => { try { fs.writeFileSync(stateFile, JSON.stringify(game)); } catch {} };
setInterval(save,5000).unref();
const room = 'game:freedom-challenge';
const now=()=>Date.now();
...
io.on('connection',socket=>{
  socket.on('join',({nickname,sessionToken}={},ack)=>{let player=Object.values(game.players).find(p=>p.token===sessionToken); if(!player){let name=safeNick(nickname);const used=new Set(Object.values(game.players).map(p=>p.nickname));let n=2,base=name;while(used.has(name))name=`${base.slice(0,16)} ${n++}`;const id=crypto.randomUUID();player={id,nickname:name,score:0,correctAnswers:0,token:crypto.randomBytes(24).toString('base64url'),socketId:socket.id};game.players[id]=player;} player.socketId=socket.id;socket.join(room);save();ack?.({ok:true,playerId:player.id,sessionToken:player.token,nickname:player.nickname,state:clientState(player.id)});emitState();});
  socket.on('host:join',({token}={},ack)=>{if(!requireHost(token,ack))return;socket.join(room);socket.data.host=true;ack?.({ok:true,state:clientState(),hostToken:token});});
